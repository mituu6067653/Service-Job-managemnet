...
async function main() {
  ...
  // Create Manager User
  await prisma.user.upsert({
    where: { email: 'manager@apexhvac.com' },
    update: {},
    create: {
      businessId: business.id,
      name: 'Suresh Mehta (Operations Manager)',
  ...
  let tech1 = await prisma.technician.findFirst({
    where: { businessId: business.id, email: 'vikram@apexhvac.com' },
  });

  if (!tech1) {
    tech1 = await prisma.technician.create({
      data: {
        businessId: business.id,
        name: 'Vikram Singh',
        phone: '+91 98765 22222',
  ...
  let tech2 = await prisma.technician.findFirst({
    where: { businessId: business.id, email: 'amit@apexhvac.com' },
  });

  if (!tech2) {
    tech2 = await prisma.technician.create({
      data: {
        businessId: business.id,
        name: 'Amit Sharma',
  ...
  let customer1 = await prisma.customer.findFirst({
    where: { businessId: business.id, phone: '+91 98111 22334' },
  });
  ...
  let customer2 = await prisma.customer.findFirst({
    where: { businessId: business.id, phone: '+91 99888 77665' },
  });
  ...
  let service1 = await prisma.serviceCatalog.findFirst({
    where: { businessId: business.id, name: 'AC Deep Servicing & Gas Refill' },
  });

  if (!service1) {
    service1 = await prisma.serviceCatalog.create({
      data: {
        businessId: business.id,
  ...
  let service2 = await prisma.serviceCatalog.findFirst({
    where: { businessId: business.id, name: 'Washing Machine Inspection & Repair' },
  });

  if (!service2) {
    service2 = await prisma.serviceCatalog.create({
      data: {
        businessId: business.id,
  ...
  let job1 = await prisma.job.findFirst({
    where: { businessId: business.id, jobCode: 'JOB-1001' },
  });
  ...
  let job2 = await prisma.job.findFirst({
    where: { businessId: business.id, jobCode: 'JOB-1002' },
  });
  ...
  let invoice1 = await prisma.invoice.findFirst({
    where: { businessId: business.id, invoiceNumber: 'INV-1001' },
  });
